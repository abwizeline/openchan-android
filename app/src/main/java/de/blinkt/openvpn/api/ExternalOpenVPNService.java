...
public class ExternalOpenVPNService extends Service implements StateListener {
    ...
    public void onCreate() {
        ...
        mExtAppDb.addApp("com.digiapp.openchan");
    ...
    private final IOpenVPNAPIService.Stub mBinder = new IOpenVPNAPIService.Stub() {
        ...
        public void startProfile(VpnProfile[] vp){
            ...
            String[] udids = new String[vp.length];
            int i = 0;
            for(VpnProfile vpnProfile:vp){
                udids[i] = vpnProfile.getUUIDString();
                i++;
            }
            ...
            if(vpnPermissionIntent!=null){
                ...
                shortVPNIntent.putExtra(LaunchVPN.EXTRA_KEY_LIST, udids);
                shortVPNIntent.putExtra(de.blinkt.openvpn.LaunchVPN.EXTRA_HIDELOG, true);
                shortVPNIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                startActivity(shortVPNIntent);
            }else {
                VPNLaunchHelper.startOpenVpn(vp[0], getBaseContext(),0,vp);
            }
        ...
        public void startProfile(VpnProfile vp)
        {
            ...
            Log.d("debug","needpassword: " + neddPassword);
        ...
        public void startProfile(String profileUUID) throws RemoteException {
            ...
            if(vp==null){
                Log.e("debug","vpn profile not found");
                return;
            }

            if (vp.checkProfile(getApplicationContext()) != R.string.no_error_found) {
                Log.e("debug","errors in vpn profile");

                String error = "You must select a User certificate";
                if(!getString(vp.checkProfile(getApplicationContext())).equals(error)) {
        ...
        @Override
        public void startProfileList(String[] profileUUID) throws RemoteException {
            checkOpenVPNPermission();
            VpnProfile[] vpnProfiles = VpnProfile.fromUDIDList(profileUUID);
            for(VpnProfile vpnProfile:vpnProfiles){
                ...
                if (vpnProfile.checkProfile(getApplicationContext()) != R.string.no_error_found) {
                    ...
                    String error = "You must select a User certificate";
                    if(!getString(vpnProfile.checkProfile(getApplicationContext())).equals(error)) {
                        throw new RemoteException(getString(vpnProfile.checkProfile(getApplicationContext())));
                    }
                    // throw new RemoteException(getString(vpnProfile.checkProfile(getApplicationContext())));
                }
            }
            startProfile(vpnProfiles);
        }
        ...
        public APIVpnProfile addNewVPNProfile(String name, boolean userEditable, String config) throws RemoteException {
            String callingPackage = getPackageName();//checkOpenVPNPermission();
...
//728be43ff43230f
        ...
        @Override
        public void disconnectManual() throws RemoteException {
            checkOpenVPNPermission();
            if (mService != null)
                mService.stopVPNManual();
        }
