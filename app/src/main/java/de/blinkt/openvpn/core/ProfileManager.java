...
public class ProfileManager {
    ...
    public static void setConntectedVpnProfileDisconnected(Context c) {
        ...
        /*
        SharedPreferences prefs = Preferences.getDefaultSharedPreferences(AppObj.getGlobalContext());
    ...
    public static void setConnectedVpnProfile(Context c, VpnProfile connectedProfile) {
        ...
        if (connectedProfile != null) {
            String uuid = connectedProfile == null ? null : connectedProfile.getName();
            tinyDB.putString(LAST_CONNECTED_PROFILE, uuid.replaceAll("\"",""));
        }
    ...
    public static VpnProfile getLastConnectedProfile(Context c) {
        TinyDB tinyDB = new TinyDB(AppObj.getGlobalContext());
        String lastConnectedProfile = tinyDB.getString(LAST_CONNECTED_PROFILE);
        if (lastConnectedProfile != null && instance!=null && instance.profiles!=null){
            for(VpnProfile vpnProfile:instance.profiles.values()){
                if(vpnProfile!=null && vpnProfile.getName()!=null){
                    if(vpnProfile.getName().replaceAll("\"","").equalsIgnoreCase(lastConnectedProfile)){
                        return vpnProfile;
                    }
                }
            }
    ...
    public void addProfile(VpnProfile profile) {
        profiles.put(profile.getUUIDString(), profile);
    ...
    private static void saveProfile(Context context, VpnProfile profile, boolean updateVersion, boolean isTemporary) {
        ...
        String filename = profile.getUUIDString().toString() + ".vp";
        if (isTemporary)
        ...
        for (String vpnentry : vlist) {
            try {
                ...
                if (vpnentry.equals(TEMPORARY_PROFILE_FILENAME)) {
                    ...
                    profiles.put(vp.getUUIDString().toString(), vp);
                }
