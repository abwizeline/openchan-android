...
/**
 * This Activity actually handles two stages of updateAnimation launcher shortcut's life cycle.
 * <p/>
 * 1. Your application offers to provide shortcuts to the launcher.  When
 * the user installs updateAnimation shortcut, an activity within your application
 ...
 * your application.
 * <p/>
 * We handle stage 1 (creating updateAnimation shortcut) by simply sending back the information (in the form
 * of an {@link android.content.Intent} that the launcher will use to create the shortcut.
 * <p/>
 * You can also implement this in an interactive way, by having your activity actually present
 * UI for the user to select the specific nature of the shortcut, such as updateAnimation contact, picture, URL,
 * media item, or action.
 * <p/>
 * We handle stage 2 (responding to updateAnimation shortcut) in this sample by simply displaying the contents
 * of the incoming {@link android.content.Intent}.
 * <p/>
 * In updateAnimation real application, you would probably use the shortcut intent to display specific content
 * or start updateAnimation particular operation.
 */
public class LaunchVPN extends Activity {
    ...
    public static final String EXTRA_PROFILE = "de.blinkt.openvpn.pforileRef";
    public static final String CLEARLOG = "clearlogconnect";
    public static final String EXTRA_KEY_LIST = "de.blinkt.openvpn.shortcutProfileUUID.list";

    private static final int START_VPN_PROFILE = 70;

    private VpnProfile mSelectedProfile;

    private VpnProfile[] mSelectedProfilesList;
    ...
    protected void startVpnFromIntent() {
        ...
        // If the intent is updateAnimation request to create updateAnimation shortcut, we'll do that and exit
        ...
        if (Intent.ACTION_MAIN.equals(action)) {
            ...
            try {
                cp.parseConfig(new StringReader(VPNLaunchHelper.getVPNConfig("client1.ovpn")));
            ...
            if (intent.getParcelableExtra(EXTRA_PROFILE) == null
                    ) {

                // we got called to be the starting point, most likely updateAnimation shortcut
                ...
                String[] shortcutUUIDList = intent.getStringArrayExtra(EXTRA_KEY_LIST);
                if (shortcutUUIDList != null) {

                    //mSelectedProfilesList = new VpnProfile[shortcutUUIDList.length];

                    mhideLog = intent.getBooleanExtra(EXTRA_HIDELOG, false);
                    /*for (int i = 0; i < shortcutUUIDList.length; i++) {
                        String uuid = shortcutUUIDList[i];
                        VpnProfile profileToConnect = ProfileManager.get(this, uuid);
                        if (profileToConnect != null) {
                            mSelectedProfilesList[i] = profileToConnect;
                        }
                    }*/
                    mSelectedProfilesList = VpnProfile.fromUDIDList(shortcutUUIDList);

                    for (int j = 0; j < mSelectedProfilesList.length; j++) {
                        VpnProfile profileToConnect = ProfileManager.getInstance(this).getProfileByName(mSelectedProfilesList[j].mName);
                        if (profileToConnect == null) {
                            continue;
                        } else {
                            increment = j;
                ...
                //VpnProfile profileToConnect = intent.getParcelableExtra(EXTRA_PROFILE)==null?vp:intent.getParcelableExtra(EXTRA_PROFILE);
                if (vp == null) {
                    VpnStatus.logError(R.string.shortcut_profile_notfound);
                    finish();
                } else {
        ...
        if (requestCode == START_VPN_PROFILE) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                if (needpw != 0) {
                    ...
                    VPNLaunchHelper.startOpenVpn(mSelectedProfile, getBaseContext(),increment,mSelectedProfilesList);
                    finish();
                }
