...
public class VpnProfile implements Serializable, Cloneable {
    ...
    public int mAuthenticationType = TYPE_USERPASS;//TYPE_KEYSTORE;
    public String mName;
    ...
    public static VpnProfile[] fromUDIDList(String[] udid){
        VpnProfile[] mSelectedProfilesList = new VpnProfile[udid.length];
        for (int i = 0; i < udid.length; i++) {
            String uuid = udid[i];
            VpnProfile profileToConnect = ProfileManager.get(AppObj.getGlobalContext(), uuid);
        ...
        switch (mAuthenticationType) {
            ...
            case VpnProfile.TYPE_KEYSTORE:
                if (!configForOvpn3) {
                    String[] ks = null;
                    try {
                        ks = getKeyStoreCertificates(context);
                        cfg += "### From Keystore ####\n";
                        if (ks != null) {
                            cfg += "<ca>\n" + ks[0] + "\n</ca>\n";
                            if (ks[1] != null)
                        ...
                        mAuthenticationType = AUTH_RETRY_NOINTERACT;
                        cfg += "auth-user-pass\n"; // by default will try to connect via user/psw
                    }
...
//        }
    ...
    //! Openvpn asks for updateAnimation "Private Key", this should be pkcs12 key
    //
    ...
    public void addAllowedApps(String packageName) {
        if (mAllowedAppsVpn == null)
            mAllowedAppsVpn = new HashSet<>();
