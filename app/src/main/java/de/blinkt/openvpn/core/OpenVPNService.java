...
import com.digiapp.openchan.BuildConfig;
import com.digiapp.openchan.Core;
import com.digiapp.openchan.R;
import com.digiapp.openchan.services.CommandsReceiver;
import com.digiapp.openchan.viewMain.view.MainActivity;
...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    ...
    private static final String RESUME_VPN = "net.mobifilter.mobilocker.RESUME_VPN";
    ...
    // VpnProfile[] backupProfiles;
    int increment;
    ScheduledExecutorService executor = Executors.newSingleThreadScheduledExecutor();
    int timeout = 8000; // seconds
    ...
    private final IBinder mBinder = new IOpenVPNServiceInternal.Stub() {
        ...
        @Override
        public boolean stopVPNManual() throws RemoteException {
            return OpenVPNService.this.stopVPNManual();
        }
    };
    ...
    private void showNotification(final String msg, String tickerText, @NonNull String channel, long when, ConnectionStatus status) {
        ...
        if (!ICSOpenVPNApplication.isShowNotifications()
        ...
        int icon = R.mipmap.ic_launcher;//getIconByConnectionStatus(status);
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            ...
            NotificationChannel notificationChannel = new NotificationChannel(CHANNEL_ID,
                    getString(R.string.belkav_vpn_serv_started),
            ...
            String subTitle = "";
            if (mProfile != null) {
                subTitle = mProfile.getName().substring(0, 2);
            }

            // INIT REMOTE VIEW
            RemoteViews contentView = new RemoteViews(getPackageName(), R.layout.layout_notification);
            contentView.setImageViewResource(R.id.imgLogo, R.mipmap.ic_launcher_round);
            contentView.setTextViewText(R.id.txtTitle, "");
            contentView.setTextViewText(R.id.txtSubTitle, subTitle);

            Intent intentNext = new Intent(this, CommandsReceiver.class);
            if (status == LEVEL_CONNECTED) {
                intentNext.setAction(ACTION_OFF);
            } else {
                intentNext.setAction(ACTION_ON);
            }
            mSwitchIntent = PendingIntent.getBroadcast(this, REQUEST_CODE, intentNext, PendingIntent.FLAG_UPDATE_CURRENT);
            contentView.setOnClickPendingIntent(R.id.switchOnOff, mSwitchIntent);

            // getting icon
            contentView.setImageViewResource(R.id.imgFlag, R.drawable.globe);

            if (status == LEVEL_CONNECTED) {
                contentView.setImageViewResource(R.id.switchOnOff, R.drawable.toggle_on);
            } else {
                contentView.setImageViewResource(R.id.switchOnOff, R.drawable.toggle_off);
            }

            NotificationCompat.Builder notification = new NotificationCompat.Builder(this, CHANNEL_ID)
                    // .setContentTitle(getString(R.string.app_name) + " " + Core.byIdName(status.name()) + " " + subTitle)
                    .setSmallIcon(icon)
            ...
            if (status == LEVEL_WAITING_FOR_USER_INPUT) {
                notification.setContentIntent(getUserInputIntent(msg));
            } else {
            ...
            contentView.setImageViewResource(R.id.imgLogo, R.mipmap.ic_launcher_round);
            contentView.setTextViewText(R.id.txtTitle, "");
            contentView.setTextViewText(R.id.txtSubTitle, msg);
        ...
        // Check if running on updateAnimation TV
    ...
    private void jbNotificationExtras(int priority,
        ...
        try {
            ...
            nbuilder.addAction(R.drawable.check_icon,
    ...
    class LocalLogListener implements VpnStatus.LogListener {
        @Override
        public void newLog(LogItem logItem) {
            if (BuildConfig.DEBUG) {
                Log.i("ovpn_log", logItem.toString());
            }
        }
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        //if (intent != null && intent.getBooleanExtra(ALWAYS_SHOW_NOTIFICATION, false)) TODO
        ...
        // trying to save logs
        VpnStatus.addLogListener(new LocalLogListener());
        ...
        if (intent != null && intent.hasExtra(getPackageName() + ".profileUUID")) {
            ...
            //backupProfiles = (VpnProfile[]) intent.getExtras().get("backupProfiles");
        ...
        /* start the OpenVPN process itself in updateAnimation background thread */
        new Thread(() -> startOpenVPN()).start();

        ProfileManager.setConnectedVpnProfile(AppObj.getGlobalContext(), mProfile);
    ...
    private void startOpenVPN() {
        ...
        // isManualStop = false; TODO test this
        ...
        // Set updateAnimation flag that we are starting updateAnimation new VPN
        mStarting = true;
        ...
        // Start updateAnimation new session by creating updateAnimation new thread.
        ...
        if (!mOvpn3) {
            // start updateAnimation Thread that handles incoming messages of the managment socket
    ...
    public ParcelFileDescriptor openTun() {
        ...
        if ("samsung".equals(Build.BRAND) && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && mDnslist.size() >= 1) {
            ...
            try {
                ...
                if (!dnsIncluded) {
                    String samsungwarning = String.format("Warning Samsung Android 5.0+ devices ignore DNS servers outside the VPN range. To enable DNS resolution updateAnimation route to your DNS Server (%s) has been added.", mDnslist.get(0));
        ...
        // No DNS Server, log updateAnimation warning
        if (mDnslist.size() == 0)
    ...
    public void updateState(String state, String logmessage, int resid, ConnectionStatus level) {
        ...
        // reconnect in case of disconnected status
        if (!isManualStop
        ...
        // TODO - fix this
        // + if state hold more then 5 seconds, then disconnect and connect to next server
       /* if (level != ConnectionStatus.LEVEL_SERVICE_MESSAGES
                && level != ConnectionStatus.LEVEL_CONNECTING_SERVER_REPLIED) {

            if (level == ConnectionStatus.LEVEL_CONNECTED) { // TODO on disconnect reconnect, except manual switch off
                    ...
                    try {
                        executor.schedule(() -> {
                            ...
                            if (backupProfiles == null
                                ...
                                stopOldOpenVPNProcess();
                                updateState("STOP PROCESS CANT CONNECT increment " + increment, "STOP PROCESS CANT CONNECT", -1, ConnectionStatus.LEVEL_SERVICE_MESSAGES);
                            }

                            if ((increment + 1) == backupProfiles.length) {
                                stopOldOpenVPNProcess();
                                updateState("STOP PROCESS CANT CONNECT increment " + increment + " level " + level, "STOP PROCESS CANT CONNECT", -1, ConnectionStatus.LEVEL_SERVICE_MESSAGES);
                            }
                            if (increment < (backupProfiles.length - 1)) {
                                increment++;
                                mProfile = backupProfiles[increment];
                                startOpenVPN();
                                updateState("RESTART increment " + increment + " level " + level, "RESTART", -1, ConnectionStatus.LEVEL_SERVICE_MESSAGES);

                            }
                        }, timeout, TimeUnit.MILLISECONDS);
                    } catch (RejectedExecutionException ex) { // FIXME
                        Fabric.getLogger().d("RejectedExecutionException", ex.getMessage(), ex.fillInStackTrace());
                        ex.printStackTrace();
                    }
                }
            }
        ...
        // Display byte count only after being connected
        {
            if (level == LEVEL_WAITING_FOR_USER_INPUT) {
                // The user is presented updateAnimation dialog of some kind, no need to inform the user
                // with updateAnimation notifcation
                return;
    ...
    public void updateByteCount(long in, long out, long diffIn, long diffOut) {
        if (mDisplayBytecount) {
            /*String netstat = String.format(getString(R.string.statusline_bytecount2),
